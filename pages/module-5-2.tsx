...
const accessModes = {
  ReadWriteOnce: { code: 'RWO', description: 'Volume can be mounted read-write by a single node', useCase: 'Most common - databases, single-node apps', color: '#22c55e' },
  ReadOnlyMany: { code: 'ROX', description: 'Volume can be mounted read-only by many nodes', useCase: 'Shared config files, static assets', color: '#0ea5e9' },
  ReadWriteMany: { code: 'RWX', description: 'Volume can be mounted read-write by many nodes', useCase: 'Shared storage (NFS), multi-node writes', color: '#f59e0b' },
};

const storageClasses = {
  fast: { name: 'fast-ssd', type: 'SSD (io2 / Premium SSD)', iops: '64,000', cost: '$$$', color: '#ef4444' },
  standard: { name: 'standard', type: 'SSD (gp3 / Standard SSD)', iops: '16,000', cost: '$$', color: '#0ea5e9' },
  archive: { name: 'archive-hdd', type: 'HDD (st1 / Standard HDD)', iops: '500', cost: '$', color: '#6b7280' },
};

const pill = (bg: string): React.CSSProperties => ({ background: bg, color: '#fff', padding: '12px 20px', borderRadius: 'var(--radius-md)', fontWeight: 600 });

export default function Module52() {
  const [storageSize, setStorageSize] = useState(10);
  const [accessMode, setAccessMode] = useState<keyof typeof accessModes>('ReadWriteOnce');
  const [storageClass, setStorageClass] = useState<keyof typeof storageClasses>('standard');

  const currentMode = accessModes[accessMode];
  const currentClass = storageClasses[storageClass];

  const modeButton = (m: keyof typeof accessModes): React.CSSProperties => ({
    padding: '10px 16px', background: accessMode === m ? accessModes[m].color : 'var(--color-bg-tertiary)',
    color: accessMode === m ? '#fff' : 'var(--color-text-secondary)', border: 'none', borderRadius: 'var(--radius-sm)',
    cursor: 'pointer', fontWeight: 600, fontSize: '0.9rem',
  });
  const classButton = (c: keyof typeof storageClasses): React.CSSProperties => ({
    padding: '10px 16px', background: storageClass === c ? storageClasses[c].color : 'var(--color-bg-tertiary)',
    color: storageClass === c ? '#fff' : 'var(--color-text-secondary)', border: 'none', borderRadius: 'var(--radius-sm)',
    cursor: 'pointer', fontWeight: 600, fontSize: '0.9rem',
  });

  return (
    <ModuleShell id="5-2" subtitle="Abstracting Storage (So Developers Don't Need to Know About AWS EBS)">
      <section className={styles.spotlight}>
        ...
        <Callout variant="danger" icon="❌" title="The Tightly-Coupled Approach">
          <TermBox>
        ...
        <Callout variant="success" icon="✅" title="The Abstracted Approach">
          <TermBox>
      ...
      <section className={styles.spotlight}>
        ...
        <Callout variant="neutral" title="Three Layers">
          <ol>
            <li>
        ...
        >
          <div style={{ fontSize: '1.2rem', fontWeight: 600, marginBottom: '16px', color: 'var(--color-text-primary)' }}>Storage Flow</div>
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
            <div style={pill('#ef4444')}>AWS EBS Volume</div>
            <div style={{ fontSize: '1.5rem', color: 'var(--color-text-muted)' }}>→</div>
            <div style={pill('#f59e0b')}>PersistentVolume</div>
            <div style={{ fontSize: '1.5rem', color: 'var(--color-text-muted)' }}>→</div>
            <div style={pill('#0ea5e9')}>PVC</div>
            <div style={{ fontSize: '1.5rem', color: 'var(--color-text-muted)' }}>→</div>
            <div style={pill('#22c55e')}>Pod</div>
          </div>
        </div>
      </section>
      ...
      <section className={styles.spotlight}>
        ...
        <p>Configure a PersistentVolumeClaim and see the generated YAML:</p>
        ...
        <div style={{ background: 'var(--color-bg-elevated)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)', padding: '24px', margin: '20px 0' }}>
          <div style={{ marginBottom: '20px' }}>
            <label style={{ display: 'block', fontWeight: 600, marginBottom: '8px', color: 'var(--color-text-primary)' }}>Storage Size: {storageSize}Gi</label>
            <input type="range" min="1" max="100" value={storageSize} onChange={(e) => setStorageSize(Number(e.target.value))} style={{ width: '100%' }} />
          </div>

          <div style={{ marginBottom: '20px' }}>
            <label style={{ display: 'block', fontWeight: 600, marginBottom: '12px', color: 'var(--color-text-primary)' }}>Access Mode:</label>
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
              <button onClick={() => setAccessMode('ReadWriteOnce')} style={modeButton('ReadWriteOnce')}>RWO (Single Node)</button>
              <button onClick={() => setAccessMode('ReadOnlyMany')} style={modeButton('ReadOnlyMany')}>ROX (Read-Only)</button>
              <button onClick={() => setAccessMode('ReadWriteMany')} style={modeButton('ReadWriteMany')}>RWX (Multi-Node)</button>
            </div>
          </div>

          <div style={{ background: 'var(--color-bg-secondary)', border: `2px solid ${currentMode.color}`, borderRadius: 'var(--radius-md)', padding: '16px', margin: '16px 0' }}>
            <div style={{ color: 'var(--color-text-primary)', fontSize: '0.9rem', marginBottom: '4px' }}><strong>{currentMode.code}:</strong> {currentMode.description}</div>
            <div style={{ color: 'var(--color-text-muted)', fontSize: '0.85rem' }}>Use case: {currentMode.useCase}</div>
          </div>

          <div style={{ marginBottom: '20px' }}>
            <label style={{ display: 'block', fontWeight: 600, marginBottom: '12px', color: 'var(--color-text-primary)' }}>Storage Class:</label>
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
              <button onClick={() => setStorageClass('fast')} style={classButton('fast')}>Fast SSD</button>
              <button onClick={() => setStorageClass('standard')} style={classButton('standard')}>Standard</button>
              <button onClick={() => setStorageClass('archive')} style={classButton('archive')}>Archive HDD</button>
            </div>
          </div>

          <div style={{ background: 'var(--color-bg-secondary)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', padding: '16px', margin: '16px 0', color: 'var(--color-text-secondary)', fontSize: '0.85rem' }}>
            <div><strong>Type:</strong> {currentClass.type}</div>
            <div><strong>IOPS:</strong> Up to {currentClass.iops}</div>
            <div><strong>Cost:</strong> {currentClass.cost}</div>
          </div>
      ...
      <section className={styles.spotlight}>
        ...
        <p>Now that the PVC is bound, you can mount it in a Pod like any other volume:</p>
      ...
      <section className={styles.spotlight}>
        ...
        <Callout variant="info" title="What Happens">
          <ol>
      ...
      <section className={styles.spotlight}>
        ...
        <div style={{ display: 'grid', gap: '1rem', margin: '1.5rem 0' }}>
          <Callout variant="success" title="ReadWriteOnce (RWO)">
            <p>
          ...
          <Callout variant="info" title="ReadOnlyMany (ROX)">
            <p>
          ...
          <Callout variant="warning" title="ReadWriteMany (RWX)">
            <p>
      ...
      <section className={styles.spotlight}>
        ...
        <p>Some StorageClasses support <strong>expanding</strong> volumes after creation:</p>
        ...
        <p>CSI drivers support <strong>snapshots</strong> (point-in-time backups):</p>
      ...
      <section className={styles.spotlight}>
        ...
        <div style={{ display: 'grid', gap: '0.75rem', margin: '1.5rem 0' }}>
          <Callout variant="danger" title='❌ PVC Stuck in "Pending"'>
            <p>
          ...
          <Callout variant="danger" title="❌ Pod Can't Mount Volume (Multi-Attach Error)">
            <p>
          ...
          <Callout variant="danger" title='❌ "Failed to Provision Volume" (Cloud Error)'>
            <p>
